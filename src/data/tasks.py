    ...
    def sample_datasets(self, datasets):
        ...
        if self.task in [
            ...
            for label in range(self.num_labels):
                data = shuffled_train.filter(
                    lambda example: int(example["label"]) == label
                )
                ...
                sampled_dev.append(
                    data.select([i for i in range(num_samples, num_samples * 2)])
                )
...
class MR(AbstractTask):
    ...
    def load_datasets(self):
        ...
        return load_dataset(
            "json", data_files=data_files, cache_dir=self.cache_dir, **dataset_args
        )
