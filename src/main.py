...
def main():
    ...
    if training_args.num_extra_tokens == -1:
        ...
        if len(processor.get_verbalizers()) != 0:
            training_args.num_extra_tokens = max(
                [len(t[0]) for t in processor.get_tokenized_verbalizers()]
            )
        else:
            ...
            def get_max_num_verbalizers_tokens(example, processor):
                max_verbalizer_length = max(
                    [len(t[0]) for t in processor.get_tokenized_verbalizers(example)]
                )
    ...
    processor = MLMProcessor(
        ...
        if not training_args.soft_pet
        else extra_token_verbalizers,
    ...
    def get_initial_weights(config, verbalizers):
        ...
        weight_init = torch.normal(
            mean=0.0,
            std=config.extra_embd_initializer_range,
    ...
    extra_embedding_weight = (
        get_initial_weights(config, verbalizers)
        if training_args.extra_tokens_init == "verbalizers"
        else None
    )
    ...
    if training_args.prompt_tune:
        assert (
            training_args.soft_pet == True and training_args.prototypical_eval
        ), "currently this is only implemented with soft_pet and prototypical_eval"
    ...
    if (
        ...
        or training_args.compute_time
        and not training_args.compute_inference_time
    ):
    ...
    if training_args.do_eval:
        ...
        eval_samples = (
            eval_dataset[0].num_rows
            if isinstance(eval_dataset, list)
            else eval_dataset.num_rows
        )
        max_eval_samples = (
    ...
    if (
        training_args.compute_memory
        or training_args.compute_time
        or training_args.compute_inference_time
    ):
        print(performance_metrics)
        trainer.save_metrics("performance", performance_metrics)
